  ...
  (function applySettings() {
    ...
    fetch(SB_URL + '/rest/v1/site_settings?select=*&id=eq.1&limit=1', { headers: { apikey: SB_KEY } })
      ...
      .then(function (rows) {
        ...
        var socials = { linkedin: s.linkedin, x: s.x_url, instagram: s.instagram, whatsapp: s.whatsapp };
        Object.keys(socials).forEach(function (k) {
          if (!socials[k]) return;
          document.querySelectorAll('[data-social="' + k + '"]').forEach(function (el) { el.href = socials[k]; });
        });

        // Contact details text (wherever [data-setting] exists)
        var texts = { email: s.email, phone: s.phone, address: s.address };
